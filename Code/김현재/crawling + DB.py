...
day = time.strftime('%Y-%m-%d %H', time.localtime(time.time())) + "시" # 년-월-일 시

# DB
conn = sqlite3.connect("C:/Users/khj40/Desktop/학교/4학년 1학기/캡스톤디자인/database.db") 
cur = conn.cursor()
...
for S_name in save_name :
    sql = "INSERT INTO stock(name) VALUES ('" + S_name + "')"
    cur.execute(sql)
    sql = "UPDATE stock SET day = '" + day + "', current = " + save_index[i] + ", tran = " + save_index[i+1] + ", start = " + save_index[i+2] + ", high = " + save_index[i+3] + ", low = " + save_index[i+4] + " WHERE name = '" + S_name + "'"
    cur.execute(sql)
    i += 5
